...
cd ~/Awsdeploy/main/Parag_StudentMgmtSystem
...
PROPERTIES_FILE=~/aleena/main/Parag_StudentMgmtSystem/src/main/resources/application.properties

DB_URL=$(grep "spring.datasource.url" $PROPERTIES_FILE | cut -d'=' -f2)
DB_USER=$(grep "spring.datasource.username" $PROPERTIES_FILE | cut -d'=' -f2)
DB_PASS=$(grep "spring.datasource.password" $PROPERTIES_FILE | cut -d'=' -f2)
...
cd ~/Awsdeploy/main/Parag_StudentMgmtSystem/target
pwd
